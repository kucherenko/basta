...
defineMode('verilog', function(config, parserConfig) {
    ...
    for (const i in noIndentKeywords) {
        const keyword = noIndentKeywords[i];
    ...
    return {
        ...
        token: function(stream, state: any) {
            ...
            if (hooks.token) {
                ...
                const style = hooks.token(stream, state);
                if (style !== undefined) {
                    return style;
                }
            }
...
defineMIME('text/x-verilog', {
    name: 'verilog'
});

defineMIME('text/x-systemverilog', {
    name: 'verilog'
});
...
const tlvFirstLevelIndentMatch = /^[! ]  /;
