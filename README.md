...
detector, and this repository carries it as a submodule pinned to one commit.
Releases are cut here, on their own schedule, and never wait for a jscpd one.
...
To release: move the submodule to the jscpd commit you want, commit that
pointer, and push a `v*` tag. The workflow checks that the tag, the crate and
the npm manifest all name the same version before it publishes anything.
