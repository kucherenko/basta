...
The release workflow already builds one `basta-<platform-key>.tar.gz` per
supported target (binary + LICENSE at the archive root). A wheel is a zip with
the same binary under `<name>-<version>.data/scripts/`, which pip and uv
install onto PATH, so there is nothing to compile here. One wheel per
tarball.

The PyPI project is `basta-cli`, because `basta` on PyPI is an unrelated
astronomy package. The command the wheels install is still `basta`.

From the repository root (release.yml runs it from there):

    python3 scripts/build-pypi-wheels.py --version 0.3.1 \
        --assets-dir release-assets --out-dir dist

    python3 scripts/build-pypi-wheels.py --pep440 0.4.0-beta.1   # -> 0.4.0b1

Only the standard library is used, so the script runs on a bare CI runner.
Adapted from kucherenko/jscpd's rust/scripts/build-pypi-wheels.py.
"""
...
PROJECT = "basta-cli"
# The distribution name as it appears in wheel and .dist-info file names.
DIST = PROJECT.replace("-", "_")
SCRIPTS = ("basta",)

# The platform keys of the release assets (jscpd/rust/npm/prebuilt-targets.json).
# glibc wheels take their manylinux floor from the binary itself (see
# `glibc_floor`), because it depends on the runner image the target was built
# on, not on anything chosen here.
PLATFORMS = {
...
SUMMARY = "Dead code detector for JavaScript, TypeScript, Vue, Svelte, Astro, Python and Rust"

LONG_DESCRIPTION = """\
# basta

basta finds code that nothing runs: unused files, exports, module-private
declarations and imports in JavaScript, TypeScript, JSX and TSX, in Vue,
Svelte and Astro components, and in Python. It reads Rust dead code from the
...
and musl, x64 and arm64) and Windows (x64, arm64). It installs the `basta`
command. There is no Python code in it and it needs no Node.js.

The name on PyPI is `basta-cli` because `basta` was taken.

## Install

```bash
pip install basta-cli          # or: pipx install basta-cli / uv tool install basta-cli
basta .

uvx --from basta-cli basta .   # run without installing
```

## pre-commit

The repository publishes a `.pre-commit-hooks.yaml`, and the hook installs
this wheel:
...
- Source, issues and the full README: https://github.com/kucherenko/basta
- The engine, which also runs as `jscpd --dead-code`: https://github.com/kucherenko/jscpd
"""
...
def pep440(version: str) -> str:
    ...
    `0.3.1` stays as is; `0.4.0-beta.1` becomes `0.4.0b1` (also alpha and rc,
...
def read_tarball(path: Path, exe: str) -> tuple[bytes, bytes]:
    """Return (binary, license) from a `basta-<key>.tar.gz` release asset."""
...
def metadata(version: str) -> bytes:
    lines = [
        ...
        "License: MIT",
        "Keywords: dead-code,unused-code,unused-exports,static-analysis,basta",
        "Project-URL: Homepage, https://github.com/kucherenko/basta",
        "Project-URL: Repository, https://github.com/kucherenko/basta",
        "Project-URL: Issues, https://github.com/kucherenko/basta/issues",
        "Project-URL: Changelog, https://github.com/kucherenko/basta/releases",
        "Requires-Python: >=3.8",
...
def build_wheel(key: str, tarball: Path, version: str, out_dir: Path) -> Path:
    ...
    scripts_dir = f"{DIST}-{version}.data/scripts"
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument("--version", help="release version, as in the basta crate")
    parser.add_argument(
        "--assets-dir", type=Path, help="directory holding basta-<key>.tar.gz"
    )
    ...
    missing = [
        k for k in PLATFORMS if not (args.assets_dir / f"basta-{k}.tar.gz").is_file()
    ]
    if missing:
        raise SystemExit(
    ...
    for key in PLATFORMS:
        wheel = build_wheel(
            key, args.assets_dir / f"basta-{key}.tar.gz", version, args.out_dir
        )
