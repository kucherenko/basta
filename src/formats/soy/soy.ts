...
defineMode('soy', function(config) {
    const textMode = getMode(config, 'text/plain');
    const modes = {
        html: getMode(config, {name: 'text/html', multilineTagIndentFactor: 2, multilineTagIndentPastTag: false}),
        attributes: textMode,
        text: textMode,
        uri: textMode,
        css: getMode(config, 'text/css'),
        js: getMode(config, {name: 'text/javascript', statementIndent: 2 * config.indentUnit})
    };
            ...
            return {
                ...
                localStates: state.localStates.map(function(localState) {
                    return {
                        ...
                        state: copyState(localState.mode, localState.state)
                    };
                })
            };
        },
...
//@TODO: registerHelper
// registerHelper("hintWords", "soy", indentingTags.concat(
